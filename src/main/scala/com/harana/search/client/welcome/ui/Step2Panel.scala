...
@react object Step2Panel {
  ...
  val component = FunctionalComponent[Unit] { _ =>
    ...
    div(
      ...
      span(className := "inline-flex w-full justify-center rounded-md pt-20 pb-15")(
        kbd(className := s"$cls text-lg px-6 py-4")("⌘"),
        kbd(className := s"$cls text-m px-8 py-4")("Space"),
      )
    )
  }
}